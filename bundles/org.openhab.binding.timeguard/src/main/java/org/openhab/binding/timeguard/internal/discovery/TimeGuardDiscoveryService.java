 ...
 */
package org.openhab.binding.timeguard.internal.discovery;
...
/**
 * The {@link TimeGuardDiscoveryService} is an implementation of a discovery service for VeSync devices. The meta-data
 * is
...
public class TimeGuardDiscoveryService extends AbstractDiscoveryService
    ...
    private @NonNullByDefault({}) TimeGuardBridgeHandler bridgeHandler;
    private @NonNullByDefault({}) ThingUID bridgeUID;

    /**
     * Creates a VeSyncDiscoveryService with enabled autostart.
     */
    public TimeGuardDiscoveryService() {
    ...
    public void setThingHandler(@Nullable ThingHandler handler) {
        if (handler instanceof TimeGuardBridgeHandler) {
            bridgeHandler = (TimeGuardBridgeHandler) handler;
    ...
    protected void startScan() {
        ...
        if (bridgeHandler != null && bridgeHandler.getThing() != null) {
    ...
    @Override
    public void handleMetadataRetrieved(TimeGuardBridgeHandler handler) {
        bridgeHandler.getDevices().map(apMeta -> {
            final Map<String, Object> properties = new HashMap<>(6);
            // final String deviceUUID = apMeta.getUuid().replace("-", "");
            final String deviceUUID = apMeta.deviceId;
            properties.put(DEVICE_PROP_NAME, apMeta.name);
            properties.put(DEVICE_PROP_CODE_VERSION, apMeta.codeVersion);
            properties.put(DEVICE_PROP_DEVICE_ID, apMeta.deviceId);

            properties.put(DEVICE_PROP_CONFIG_DEVICE_ID, apMeta.deviceId);
            properties.put(DEVICE_PROP_CONFIG_DEVICE_NAME, apMeta.name);

            return DiscoveryResultBuilder.create(new ThingUID(THING_TYPE_DEVICE, bridgeUID, deviceUUID))
                    .withLabel(apMeta.name).withBridge(bridgeUID).withProperties(properties).build();
