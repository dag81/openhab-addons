...
import org.openhab.binding.timeguard.internal.dto.requests.TimeGuardSetModeRequest;
import org.openhab.binding.timeguard.internal.dto.responses.TimeGuardGetModeResponse;
...
/**
 * The {@link TimeGuardDeviceHandler} is responsible for handling commands, which are
 * sent to one of the channels.
 *
 * @author David Goodyear - Initial contribution
 */
@NonNullByDefault
public class TimeGuardDeviceHandler extends TimeGuardBaseDeviceHandler {

    public static final int DEFAULT_OPERATING_MODE_POLL_RATE = 120;
    // "Device Type" values
    ...
    private final Logger logger = LoggerFactory.getLogger(TimeGuardDeviceHandler.class);
    ...
    public TimeGuardDeviceHandler(Thing thing) {
        super(thing);
    }
    ...
    @Override
    public void updateBridgeBasedPolls(final TimeGuardBridgeConfiguration config) {
        Integer pollRate = config.operatingModePollRate;
        if (pollRate == null) {
            pollRate = DEFAULT_OPERATING_MODE_POLL_RATE;
        }
    ...
    protected boolean isDeviceSupported() {
        // No specific filtering required
        return true;
    }
        ...
        scheduler.submit(() -> {
            if (command instanceof StringType) {
                switch (channelUID.getId()) {
                    case CHANNEL_DEVICE_WORK_MODE:
                        sendCommand(HttpMethod.PUT, devicePutMode, new TimeGuardSetModeRequest(command.toString()));
                        break;
                }
            }
            performReadbackPoll();
    ...
    protected void pollForDeviceData(final ExpiringCache<String> cachedResponse) {
        ...
        synchronized (pollLock) {
            ...
            operationModeResponse = TimeGuardBindingConstants.GSON.fromJson(response, TimeGuardGetModeResponse.class);

            if (operationModeResponse == null) {
                return;
            }
        ...
        updateState(CHANNEL_DEVICE_WORK_MODE, new StringType(operationModeResponse.message.workMode));
         ...
         * OnOffType.from(humidifierStatus.result.result.waterTankLifted));
